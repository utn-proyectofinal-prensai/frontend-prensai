...
export default function AdminPage() {
  ...
  const handleMencionSubmit = (e: React.FormEvent) => {
    ...
    const nuevaMencion: Mencion = {
      id: editingMencion?.id || Date.now().toString(),
      nombre: formData.get('nombre') as string,
      activo: true
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {menciones.map((mencion) => (
                <div key={mencion.id} className="bg-black/30 backdrop-blur-sm rounded-xl border border-white/20 p-6">
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="text-lg font-semibold text-white">{mencion.nombre}</h3>
                    <div className="flex space-x-2">
                      <button
                  ...
                  <div className="space-y-2">
                    <p className="text-white/80 text-sm">
                      <span className="font-medium">Nombre a buscar:</span> {mencion.nombre}
                    </p>
                  </div>
                  <div className="mt-4 flex items-center space-x-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      {mencion.activo ? 'Activo' : 'Inactivo'}
                    </span>
                  </div>
                </div>
              ))}
